...
function banner(storeName) {
  ...
  console.log(chalk.blue.bold('  ║') + chalk.white.bold('   SHOPIFY THEME PUSH                  ') + chalk.blue.bold('║'));
  console.log(chalk.blue.bold('  ║') + chalk.gray('   Safe push workflow                   ') + chalk.blue.bold('║'));
  console.log(chalk.blue.bold('  ╚═══════════════════════════════════════╝'));
  console.log('');
...
async function main() {
  ...
  console.log('');
  divider();
  console.log(chalk.red.bold('  LIVE THEME (protected - cannot push):'));
  ...
  if (devThemes.length === 0) {
    ...
    info('Create a new theme in Shopify admin first');
    process.exit(1);
  }
  ...
  const selectedTheme = await select({
    message: chalk.cyan('Select a theme to push to:'),
    choices,
    pageSize: 10
  });
  ...
  // Step 6: Double confirmation for push
  console.log(chalk.yellow('  ⚠ WARNING: This will overwrite remote theme files'));
  console.log('');

  const proceed = await confirm({
    message: chalk.yellow(`Push to "${selectedTheme.name}"?`),
    default: false
  });
  ...
  try {
    await pushToTheme(store, password, selectedTheme.id);
    console.log('');
